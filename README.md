...
DockCatWin 的原创概念、行为设计以及默认栗子资源均来自 DockCat 项目。当前移植已同步 DockCat `v0.7` 的主要中文体验与资源内容。
...
- 支持喝水提醒、起身活动提醒、自定义提醒，以及出门/专注模式。
- 支持自定义小猫资源包；不完整的资源包会自动回退到默认栗子。
- 会记录本地使用统计和小猫出门带回的收藏品。
- 支持通过兑换码领取特殊收藏品。
- 支持自动备份用户数据，也可以从备份文件恢复设置、统计和收藏品。
...
- 隐藏小猫后，可以从系统托盘菜单重新显示。
- 可从菜单中手动切换散步和休息状态。
- 可从小猫右键菜单或系统托盘菜单开启/关闭开机自启。
- 设置、资源包和存档默认保存在程序同目录的 `UserData`，方便便携使用和备份。
- 资源包可为抱起姿态单独设置显示尺寸。
- 自定义资源包可选使用本机已安装的 `ffmpeg` 从绿幕视频生成走路序列帧。

当前暂未包含 DockCat `v0.7` 的英文界面切换；DockCatWin 的界面和文档仍以中文为主。
...
- 设置中可修改小猫名字、对你的称呼、显示器、显示缩放、提醒间隔、提醒文案、状态时长和默认出门时间，也可以兑换收藏品。
- 隐藏小猫后，可以通过系统托盘菜单重新显示。
- 勾选菜单中的“开机自启”后，DockCatWin 会随当前 Windows 用户登录自动启动；取消勾选即可关闭。
...
这些数据默认保存在程序同目录的 `UserData` 文件夹中。更新 DockCatWin 时，请保留这个文件夹，小猫就能继续读取原来的设置和存档。DockCatWin 也会在 `UserData\DataBackup` 中维护一份 `user-data-backup.json`，可从右键菜单或系统托盘菜单的“恢复备份...”导入。
